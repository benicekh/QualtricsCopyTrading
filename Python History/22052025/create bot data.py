...
for itera in range(1,3): 
        ...
        for ch in [0.15]:
            ...
            for start_col in range(0, num_cols, batch_size):
                ...
                for col in range(start_col, end_col):
                    ...
                    stringp += "]\n"
                count = i + (10*(itera-1))
                file_name = f"javastrings_price_series_{count}.txt"
    ...
    data = create_bots(len(etas),10, column_names)
    ...
    for x, (key, value) in enumerate(data.items()):
        ...
        if outputtxt ==1:
            ...
            if itera == 1:
                ...
                count = int(series) + (10*(itera-1))
            if int(series) > 5:
                file_name = f"javastrings_bots_series_low_{count}.txt"
            else:
                file_name = f"javastrings_bots_series_high_{count}.txt"
            subfolder = 'Botdata'
            file_path = os.path.join(subfolder, file_name)
            file_path = os.path.join(folder_path, file_path)
        ...
        for stage in range(1, 11):
            ...
            for player in range(1, len(etas) + 1):
                ...
                if "ResponseId" not in df.columns or "Rank" not in df.columns:
                    print(f"Missing required columns in iteration {itera}, {stage_key}, {player_key}")
                    break
                ...
                if response_id in response_ranks:
                    print(f"Duplicate ResponseId '{response_id}' found in iteration {itera}, {stage_key}, {player_key}")
                    break
