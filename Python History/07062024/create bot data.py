...
for itera in range(1,2): 
    ...
    for q in [0.15]:
        ...
        for ch in [0.15]:
            ...
            for col in range(selected_S.shape[1]):
                pathname = f"pricePaths.path{col}"
                stringp += f"{pathname} = ["
                # Loop over the rows
    ...
    'wealthALL', 'Treatment', 'roundSeries', 'priceSeries', 'assetsSeries',
    'ongoingReturnSeries', 'phaseWealth', 'GainCalc', 'realizedReturnCalc',
    ...
    for i in range(1,11):
        series_name = f'{i}'
        serie = i
        data[series_name] = populate_dataframes(data[series_name], T, 11, Scopy)
        Scopy = Scopy[:T, 11:]
        for i, (key, value) in enumerate(assetsCopy.items()):
            ...
            data[series_name][player_name]['a'] = value["1.0"][:T, :11].reshape(-1, order="F")
            data[series_name][player_name]['a_diff'] = value["1.0"][:T, :11].reshape(-1, order="F")
            for i in range(0, 440, 40):
                #changes in assets held for further calculations
                batch = data[series_name][player_name]['a'][i:i+40]
                
                difference_array = np.diff(batch, axis=0)
                difference_array = np.insert(difference_array, 0, 0, axis=0)
                
                data[series_name][player_name]['a_diff'][i:i+40] = difference_array
                ...
                path = f'path{count}'
                pathindex = count-1
                data[series_name][player_name]['path'][i:i+40] = path
                data[series_name][player_name]['good'][i:i+40] = datadump['good_bad_binary'][pathindex]
                
                
            value["1.0"] = value["1.0"][:T, 11:]
            
        for i, (key, value) in enumerate(cashsCopy.items()):
            player_name = f'player_{i+1}'
            data[series_name][player_name]['currWealth'] = value["1.0"][:T, :11].reshape(-1, order="F")
            ...
            for i in range(0, 440, 40):
                ...
                batch = data[series_name][player_name]['currWealth'][i:i+40]
                ongoingReturn = ((batch/c)-1)*100
                data[series_name][player_name]['ongoingReturn'][i:i+40] = np.round(ongoingReturn, decimals=2)
            for i in range(40, 440, 40):
                ...
                if i==40 :
                    ...
                    lastCalc = last.iloc[0]    
                batch = data[series_name][player_name]['GainCalc'][i:i+40]
                gainAll = batch + lastCalc
                data[series_name][player_name]['gainAll'][i:i+40] = gainAll
                batch = data[series_name][player_name]['p'][i-40:i]
                price_list = batch.tolist()
                price_string = '[{}]'.format(', '.join(map(str, price_list)))
                data[series_name][player_name]['priceSeries'][i:i+40] = price_string
                batch = data[series_name][player_name]['a'][i-40:i]
                asset_list = batch.tolist()
                asset_string = '[{}]'.format(', '.join(map(str, asset_list)))
                data[series_name][player_name]['assetsSeries'][i:i+40] = asset_string
                batch = data[series_name][player_name]['r'][i-40:i]
                round_list = batch.tolist()
                round_string = '[{}]'.format(', '.join(map(str, round_list)))
                data[series_name][player_name]['roundSeries'][i:i+40] = round_string
                batch = data[series_name][player_name]['ongoingReturn'][i-40:i]
                return_list = batch.tolist()
                return_list = [round(num, 2) for num in return_list]
                return_string = '[{}]'.format(', '.join(map(str, return_list)))
                data[series_name][player_name]['ongoingReturnSeries'][i:i+40] = return_string
                
                lastWealth = data[series_name][player_name]['currWealth'][i-1:i]
                lastWealth_val = lastWealth.iloc[0]
                data[series_name][player_name]['phaseWealth'][i:i+40] = lastWealth_val
                if i==40 :
                ...
                batch = data[series_name][player_name]['currWealth'][i:i+40]
                gainAll = batch + lastCalc
                data[series_name][player_name]['wealthALL'][i:i+40] = gainAll
                
                lastRet = data[series_name][player_name]['ongoingReturn'][i-1:i]
                lastRet_val = lastRet.iloc[0]
                data[series_name][player_name]['phaseReturn'][i:i+40] = lastRet_val
                
                lastGain = data[series_name][player_name]['GainCalc'][i-1:i]
                lastGain_val = lastGain.iloc[0]
                data[series_name][player_name]['gain'][i:i+40] = lastGain_val
                
                stage = i/T
                endow = c*(stage+1)
                batch = data[series_name][player_name]['wealthALL'][i:i+40]
                ret = ((batch/endow)-1)*100
                data[series_name][player_name]['returnAll'][i:i+40] = round_var(ret)
            for i in range(0, 440, 40):
                portfolio = data[series_name][player_name]['unrealized'][i:i+40]
                assets = data[series_name][player_name]['a'][i:i+40]
                assetChange = data[series_name][player_name]['a_diff'][i:i+40]
                price = data[series_name][player_name]['p'][i:i+40]
                for index, value in portfolio.items():
                    if index == i:
                       portfolio[index] = price[index]*assetChange[index]
                        ...
                        if assets[index] == 0:
                            portfolio[index] = 0
                data[series_name][player_name]['unrealized'][i:i+40] = round_var(portfolio)
                portfolioReturn = data[series_name][player_name]['portfolio'][i:i+40]
                for index, value in portfolio.items():
                    if assets[index] != 0:
    ...
    for x, (key, value) in enumerate(data.items()):
        ...
        # Sorting/Ranking 0 - ongoingReturn, 1 - Rank, 2 - ResponseID, 3 - gainAll, 4 - returnAll, 5 - pathseries
