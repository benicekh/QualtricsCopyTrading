...
os.chdir(r"C:\Users\U338144\surfdrive - Hiepler, B. (Benjamin)@surfdrive.surf.nl\Projects\Copy Trading\Qualtrics Exp\Python - Ordered List")
file_count = 1  
...
for x in range(1, 6):
    ...
    val = f"{x}"
    headers = df.iloc[:2]
    data_rows = df.iloc[2:].loc[df['pathseries'] == val].reset_index(drop=True)
    df = pd.concat([headers, data_rows]).reset_index(drop=True)
    ...
    for i in range(2, len(df)):
        ...
        risk_summary = data_players[variable_name].groupby(["stg", "phase"])['a'].mean().reset_index()
        value_to_delete =  'training'
        condition = risk_summary['phase'] == value_to_delete
        risk_summary = risk_summary.drop(risk_summary[condition].index)
        risk_summary["risk"]=risk_summary["a"]
        risk_summary = risk_summary.drop(columns='a', axis=1)
        data_players[variable_name] = data_players[variable_name].merge(risk_summary, on = 'stg', how='left')
        ...
        columns_to_move = ["ongoingReturn", "Rank", "ResponseId", "gainAll", "returnAll", "pathseries"] 
        new_order = columns_to_move + [col for col in  data_players[variable_name].columns if col not in columns_to_move]
        data_players[variable_name] =  data_players[variable_name][new_order]
    ...
    for key, df in data_players.items():
        csv_filename = f"{output_folder}/player_{file_count}.csv"  # Define the full path to the CSV file
        file_count += 1
    ...
    dfOutput.to_csv( f"pilotJSdata2310partial-series{x}.csv", index=False, encoding='utf-8-sig')
    ...
    file_name = f"stringPilot2310partial-series{x}.txt"        
