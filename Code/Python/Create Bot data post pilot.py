...
for batch_idx, start_col in enumerate(range(0, num_cols, batch_size)):
    # clean up your key (spaces and hyphens → underscores)
    key_label = final_selection_keys[batch_idx].replace(' ', '_').replace('-', '_')
    
    end_col = min(start_col + batch_size, num_cols)
    
    # build up each file’s contents
    lines = ["window.pricePaths = {}"]
    for col in range(start_col, end_col):
        # e.g. “pricePaths.path5 = [ … ]”
        vals = ",".join(str(selected_S[row, col]) for row in range(selected_S.shape[0]))
        lines.append(f"pricePaths.path{col+1} = [{vals},]")
    body = "\n".join(lines) + "\n"
    
    # write both .txt and .js versions
    for ext in ('.txt', '.js'):
        fname = f"javastrings_price_series_{key_label}{ext}"
        fpath = os.path.join(folder_path, subfolder, fname)
        export_string_to_txt_file(body, fpath)
...
for batch_idx, start_col in enumerate(range(0, num_cols, batch_size)):
    ...
    # only take the first two columns of each batch
    cols_to_export = range(start_col, min(start_col + 2, end_col))
    
    # build up each file’s contents
    lines = ["window.pricePathsCRRA = {}"]
    for col in cols_to_export:
        vals = ",".join(str(selected_S[row, col]) for row in range(selected_S.shape[0]))
        lines.append(f"pricePathsCRRA.path{col+1} = [{vals},]")
    body = "\n".join(lines) + "\n"
...
np.savetxt(file_path, selected_S, delimiter=',', fmt='%d')
...
for i in range(1,9):
    ...
    for i, (key, value) in enumerate(assetsCopy.items()):
        ...
        for i in range(0, RoT, Ro):
            ...
            val = datadump['type'][serie - 1]
            quoted = f'"{val}"'   
            data[series_name][player_name]['type_path'][i:i+Ro] = quoted
    ...
    for i, (key, value) in enumerate(cashsCopy.items()):
        ...
        for i in range(Ro, RoT, Ro):
            ...
            if i >= 2 * Ro:
                ...
                #print("Raw:", ret)
                #print("Rounded:", round_var(ret))
                if i == RoT - Ro:
                    endow = c*(stage)
                    batch = data[series_name][player_name]['wealthAllv2'][i+(Ro-1):i+Ro]
                    ret = (((batch)/endow)-1)*100
                    data[series_name][player_name]['returnAllv2'][i+(Ro-1):i+Ro] = round_var(ret)
...
for x, (key, value) in enumerate(data.items()):
    ...
    if outputtxt ==1:
        ...
        typepath = typepath.replace(' ', '_')  # Clean up spaces if needed
        typepath = typepath.replace('-', '_')  # minus to underscore
        typepath = typepath.replace('"', '')  # minus to underscore
        ...
        file_name = f"javastrings_bots_{typepath}.js"
        subfolder = 'Botdata'
        file_path = os.path.join(subfolder, file_name)
        file_path = os.path.join(folder_path, file_path)
        export_string_to_txt_file(string, file_path)
