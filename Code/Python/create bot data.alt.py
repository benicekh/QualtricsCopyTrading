...
folder_path = r"D:\Sync\Projects\Copy Trading\Trading game"
...
for itera in range(1,3): 
    ...
    for i in range(1,11):
        ...
        for i, (key, value) in enumerate(assetsCopy.items()):
            ...
            for i in range(0, RoT, Ro):
                ...
                count = int((((i/40)+(int(series_name_base)-1)*10)+1)+(int(series_name_base)-1))
                path = f'path{count}'
                pathindex = count-1
    ...
    for x, (key, value) in enumerate(data.items()):
        ...
        for i in range(11):
            ...
            # Find the minimum and maximum 'ongoingReturn' across all 5 players in the current stage
            min_return_stage = min(round_data[f"player_{j}"]["ongoingReturn"].min() for j in range(1, 6))
            max_return_stage = max(round_data[f"player_{j}"]["ongoingReturn"].max() for j in range(1, 6))
            
            # Append the min and max values for the current stage to the respective lists
            min_ongoing_returns.append(min_return_stage)
            max_ongoing_returns.append(max_return_stage)
        
        # Store the min and max lists for the current series-itera combination in the results dictionary
        results[(series, itera)] = {
